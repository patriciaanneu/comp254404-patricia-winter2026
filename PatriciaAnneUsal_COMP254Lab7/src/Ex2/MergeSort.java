...
class MergeSort {
  ...
  public static <K> void mergeSortBottomUp(Queue<K> S, Comparator<K> comp) {
    ...
    Queue<Queue<K>> groups = new LinkedQueue<>();

    //place each item into its own queue
    while (!S.isEmpty()) {
      Queue<K> single = new LinkedQueue<>();
      single.enqueue(S.dequeue());
      groups.enqueue(single);
    }

    //repeatedly merge pairs of queues
    while (groups.size() > 1) {
      Queue<K> first = groups.dequeue();
      Queue<K> second = groups.dequeue();
      Queue<K> merged = new LinkedQueue<>();

      merge(first, second, merged, comp);
      groups.enqueue(merged);
    }

    //move the sorted result back into S
    Queue<K> result = groups.dequeue();
